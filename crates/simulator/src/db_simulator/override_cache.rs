...
use sui_types::storage::{ConfigStore, FullObjectKey, InputKey, MarkerValue, ObjectKey, ObjectOrTombstone, PackageObject};
...
impl ChildObjectResolver for OverrideCache {
    ...
    fn get_object_received_at_version(
        ...
        let full_object_id = FullObjectID::new(*receiving_object_id, Some(receive_object_at_version));
        if recv_object.owner != Owner::AddressOwner((*owner).into())
            || self.have_received_object_at_version(FullObjectKey::new(full_object_id, receive_object_at_version), epoch_id)
        {
            return Ok(None);
        }
...
impl ConfigStore for OverrideCache {
    fn get_current_epoch_stable_sequence_number(&self, object_id: &ObjectID, epoch_id: EpochId) -> Option<VersionNumber> {
        todo!()
    }
}
...
impl ObjectCacheRead for OverrideCache {
    ...
    fn notify_read_input_objects<'a>(&'a self, input_and_receiving_keys: &'a [InputKey], receiving_keys: &'a HashSet<InputKey>, epoch: &'a EpochId) -> futures_core::future::BoxFuture<'a, ()> {
        todo!()
    }
